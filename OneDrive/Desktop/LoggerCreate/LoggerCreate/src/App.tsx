...
const BASE_URL_API = "https://website-production-8f27.up.railway.app";

async function lookupRobloxUser(username: string): Promise<{ id: number; name: string; avatarUrl: string | null }> {
  const res = await fetch(`${BASE_URL_API}/api/roblox-user`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (res.status === 404) {
    ...
    throw new Error(d.error ?? `No Roblox user found with username "${username}"`);
  }
  if (res.status === 429) throw new Error("Rate limited — wait a few seconds and try again.");
  if (!res.ok) {
    const d = await res.json().catch(() => ({}));
    throw new Error(d.error ?? `Server error (${res.status}) — try again.`);
  }
  const data = await res.json();
  return { id: data.id, name: data.name, avatarUrl: data.avatarUrl ?? null };
}
